...
<?php
	if(!empty($_GET['sem']))
	{
		$res="SELECT subname FROM subject WHERE department = '".$_GET['dept']."' and batch = ".$_GET['batch']." and semester = ".$_GET['sem']." ";
		$result=mysqli_query($con,$res) or die(mysqli_error($con));
...
<?php
	...
	if(!empty($_GET))
	{
		...
		{			
			...
			$subqry=mysqli_fetch_array($subres);
			$rows=unserialize($subqry['subname']);
			if($num>0)
			{
...
?>
<tr><td><?php echo $row['regno'] ?></td><td><?php echo $row['rollno'] ?></td><td><?php echo $row['name'] ?></td><td><input type="text" name="rollno[]" value="<?php echo $row['rollno']; ?>" style="display:none"><input type='text' name='mark[]' id='mark' ></td></tr>
<?php
